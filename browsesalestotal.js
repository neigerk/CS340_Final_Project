module.exports = function(){
    ...
    function getSales(res, mysql, context, complete){
        mysql.pool.query("SELECT Orders.date, SUM(Orders.order_price) AS Total_Revenue FROM Orders GROUP BY Orders.date", function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
    ...
    function getSalesbyDate(req, res, mysql, context, complete){
      ...
      var query = "SELECT Orders.date, SUM(Orders.order_price) AS Total_Revenue FROM Orders WHERE Orders.date BETWEEN '" + format_start + "' AND '"+ format_end + "' GROUP BY Orders.date"
      console.log(query);
    ...
    router.get('/', function(req, res){
        ...
        function complete(){
            callbackCount++;
            if(callbackCount >= 1){
                res.render('browsesalestotal', context);
            }
